...
if [ -f "$BACKEND_ENV" ]; then
    echo -e "${YELLOW}⚠️  Backend .env already exists at $BACKEND_ENV${NC}"
...
if [ -f "$FRONTEND_ENV" ]; then
    echo -e "${YELLOW}⚠️  Frontend .env.local already exists at $FRONTEND_ENV${NC}"
    read -p "Do you want to overwrite it? (y/N): " -n 1 -r
    echo
    if [[ ! $REPLY =~ ^[Yy]$ ]]; then
...
if [ -z "$BACKEND_SKIP" ]; then
    ...
    if [ -f "backend/env.example" ]; then
        ...
        echo -e "${GREEN}✅ Created $BACKEND_ENV from env.example${NC}"
        echo ""
        echo "⚠️  IMPORTANT: Edit $BACKEND_ENV and configure:"
...
KIMI_API_KEY=
EOF
        echo -e "${GREEN}✅ Created minimal $BACKEND_ENV${NC}"
    fi
fi
...
if [ -z "$FRONTEND_SKIP" ]; then
    ...
    if [ -f "frontend/env.example" ]; then
        ...
        echo -e "${GREEN}✅ Created $FRONTEND_ENV from env.example${NC}"
        echo ""
        echo "⚠️  IMPORTANT: Edit $FRONTEND_ENV and configure:"
        ...
        echo -e "${YELLOW}⚠️  frontend/env.example not found. Creating minimal .env.local...${NC}"
...
EOF
        echo -e "${GREEN}✅ Created minimal $FRONTEND_ENV${NC}"
    fi
fi
    ...
    if command -v python3 &> /dev/null; then
        ...
        if [ -n "$SECRET_KEY" ] && [ -n "$ENCRYPTION_KEY" ]; then
            if [ -f "$BACKEND_ENV" ]; then
                ...
                echo -e "${GREEN}✅ Updated SECRET_KEY and ENCRYPTION_KEY in $BACKEND_ENV${NC}"
            else
            ...
            echo -e "${YELLOW}⚠️  Could not generate keys. Make sure Python 3 and cryptography are installed.${NC}"
            echo "   Install: pip install cryptography"
        fi
    else
        echo -e "${YELLOW}⚠️  Python 3 not found. Skipping key generation.${NC}"
